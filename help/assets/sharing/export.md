...
O script, gravado como um módulo JavaScript, faz parte de um projeto Node.js, pois tem uma dependência em `node-fetch` e `p-limit`. Você pode copiar o script abaixo em um projeto Node.js vazio do tipo `module` e executar `npm install node-fetch p-limit` para instalar a dependência.
