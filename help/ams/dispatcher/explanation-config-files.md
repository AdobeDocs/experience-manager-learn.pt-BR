...
| NOME DO ARQUIVO`.vhost` | Preparado: `/etc/httpd/conf.d/available_vhosts/`<br>Ativo: `/etc/httpd/conf.d/enabled_vhosts/`<br/><br/><b>Nota:</b> Os arquivos .vhost não devem ser copiados para a pasta enabled_vhosts, mas use symlinks para um caminho relativo para o arquivo available_vhosts/\*.vhost</u><br><br> | Os arquivos \*.vhost (Host Virtual) são `<VirtualHosts>`  entradas para corresponder aos nomes de host e permitir que o Apache manipule cada tráfego de domínio com regras diferentes. No `.vhost` arquivo, outros arquivos como `rewrites`, `whitelisting`, `etc` serão incluídos. |
