---
title: Criar um componente personalizado | Introdução ao Editor e Angular de SPA do AEM
...
5. Atualize a variável estática `RESOURCE_TYPE` para apontar para o componente AEM `wknd-spa-angular/components/custom-component` criado no exercício anterior.
   ...
   O tipo de recurso do componente é o que vinculará o Modelo do Sling ao componente do AEM e será mapeado ao componente Angular.
...
6. Adicione o método `getExportedType()` à classe `CustomComponentImpl` para retornar o tipo de recurso do componente:
   ...
   Esse método é necessário ao implementar a interface `ComponentExporter` e exporá o tipo de recurso que permite o mapeamento para o componente Angular.
...
O código Angular do Componente Personalizado já foi criado. Em seguida, faça algumas atualizações para mapear o componente Angular para o componente do AEM.
...
5. Abra `custom.component.css` e adicione a seguinte regra:
   ...
   Para que o Espaço reservado do Editor AEM seja exibido corretamente quando o componente estiver vazio, o `:host-context` ou outro `<div>` precisa ser definido como `display: block;`.

6. Implante todas as atualizações em um ambiente AEM local a partir da raiz do diretório do projeto, usando suas habilidades Maven:
...
Parabéns, você aprendeu a criar um componente personalizado do AEM e como os Modelos e diálogos do Sling funcionam com o modelo JSON.
