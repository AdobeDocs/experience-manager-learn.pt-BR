...
Este capítulo adicionará um componente simples `Header` ao SPA. No processo de criação desse componente estático `Header`, várias abordagens para AEM desenvolvimento SPA serão usadas.
...
3. Em `dependencies` você deve ver vários relacionados a `@angular`:
   ...
   O módulo `ui.frontend` é um [aplicativo Angular](https://angular.io) gerado usando a [ferramenta Angular CLI](https://angular.io/cli) que inclui roteamento.
...
8. Inspect o arquivo `app.component.ts` em `ui.frontend/src/app/app.component.ts`:
   ...
   `app.component.js` é o ponto de entrada do SPA. `ModelManager` é fornecido pelo AEM Editor JS SDK. É responsável por chamar e injetar o `pageModel` (o conteúdo JSON) no aplicativo.
...
2. Instalar [Angular CLI](https://angular.io/cli#installing-angular-cli) globalmente Isso é usado para gerar componentes do Angular, bem como para criar e servir o aplicativo do Angular por meio do comando **ng**.
...
5. Abra o arquivo `header.component.html` e substitua o conteúdo pelo seguinte:
   ...
   Observe que isso exibe o conteúdo estático, de modo que esse componente de Angular não requer ajustes no `header.component.ts` padrão gerado.
...
Um [servidor de desenvolvimento de webpack](https://webpack.js.org/configuration/dev-server/) pode ser usado para desenvolver rapidamente o SPA. O SPA é conduzido por um modelo JSON gerado pelo AEM. Neste exercício, o conteúdo JSON de uma instância em execução do AEM será **proxied** no servidor de desenvolvimento configurado pelo [Angular project](https://angular.io/guide/build).
...
1. Retorne ao IDE e abra o arquivo **proxy.conf.json** em `ui.frontend/proxy.conf.json`.
   ...
   O [Angular app](https://angular.io/guide/build#proxying-to-a-backend-server) fornece um mecanismo fácil para solicitações de API de proxy. Os padrões especificados em `context` são enviados por proxy por meio de `localhost:4502`, o início rápido do AEM local.
...
5. Abra o arquivo **angular.json**. Adicione uma nova configuração **dev** com uma matriz **assets** atualizada para fazer referência à pasta **mocks** criada.
   ...
   Criar uma configuração dedicada **dev** garante que a pasta **mocks** seja usada somente durante o desenvolvimento e nunca seja implantada em AEM em uma build de produção.
...
1. Abra uma janela de terminal e pare o **servidor dev do webpack**, se iniciado. Dentro da pasta `ui.frontend`, digite o seguinte comando para atualizar o aplicativo do Angular para processar arquivos **.scss**.
...
As alterações feitas no **Cabeçalho** são atualmente visíveis apenas por meio do **servidor dev do webpack**. Implante o SPA atualizado para AEM para visualizar as alterações.
...
Parabéns, você atualizou o SPA e explorou a integração com o AEM! Agora você sabe de duas abordagens diferentes para desenvolver o SPA em relação à API do modelo JSON AEM usando um **servidor dev do webpack**.

Você sempre pode visualizar o código concluído em [GitHub](https://github.com/adobe/aem-guides-wknd-spa/tree/Angular/integrate-spa-solution) ou verificar o código localmente ao alternar para a ramificação `Angular/integrate-spa-solution`.

### Próximas etapas {#next-steps}

[Mapear componentes de SPA para AEM componentes](map-components.md)  - saiba como mapear componentes do Angular para componentes do Adobe Experience Manager (AEM) com o SDK JS do AEM Editor SPA. O mapeamento de componentes permite que os autores façam atualizações dinâmicas em componentes SPA no Editor de SPA de AEM, de forma semelhante à criação de AEM tradicional.
