...
+ Um ambiente AEM as a Cloud Service tem uma Credenciais de Serviço que mapeia para uma conta técnica AEM usuário. As Credenciais de Serviço não podem ser usadas para autenticar no mesmo ambiente AEM as a Cloud Service de diferentes usuários AEM conta técnica.

As Credenciais de Serviço e os tokens de acesso que geram, bem como os Tokens de Acesso ao Desenvolvimento Local, devem ser mantidos secretos, pois todos os três podem ser usados para obter acesso aos respectivos ambientes AEM as a Cloud Service
...
As credenciais de serviço, ao contrário dos tokens de acesso de desenvolvimento local, exigem uma _inicialização única_ pelo Administrador do Adobe Org IMS antes de baixá-los.

![Inicializar Credenciais de Serviço](assets/service-credentials/initialize-service-credentials.png)

__Esta é uma inicialização única por ambiente as a Cloud Service AEM__

1. Verifique se você está conectado como:
   + Administrador da sua organização do Adobe IMS
   + Membro da __Cloud Manager - Desenvolvedor__ Perfil de produto IMS
   + Membro da __Usuário AEM__ ou __Administradores de AEM__ Perfil de produto IMS em __Autor do AEM__
...
Baixar as credenciais de serviço segue as mesmas etapas da inicialização. Se a inicialização ainda não tiver ocorrido, o usuário receberá um erro ao tocar no botão __Obter Credenciais de Serviço__ botão.

1. Verifique se você está conectado como um:
   + Membro da __Cloud Manager - Desenvolvedor__ Perfil de produto IMS (que concede acesso ao Console do desenvolvedor AEM)
      + Os ambientes de sandbox AEM as a Cloud Service não exigem isso  __Cloud Manager - Desenvolvedor__ associação
   + Membro da __Usuário AEM__ ou __Administradores de AEM__ Perfil de produto IMS em __Autor do AEM__
1. Faça logon em [Adobe Cloud Manager](https://my.cloudmanager.adobe.com)
...
1. Um aplicativo externo precisa interagir programaticamente com AEM ambientes as a Cloud Service
...
Para acessar AEM as a Cloud Service usando as credenciais de serviço, nosso aplicativo externo deve ser atualizado de 3 maneiras:
...
1. Troque o JWT por um token de acesso
   + Quando as Credenciais de Serviço estão presentes, nosso aplicativo externo usa esse token de acesso em vez do Token de Acesso de Desenvolvimento Local, ao acessar AEM as a Cloud Service

Neste tutorial, Adobe `@adobe/jwt-auth` o módulo npm é usado para ambos, (1) gerar o JWT a partir das Credenciais de Serviço e (2) trocá-lo por um token de acesso, em uma única chamada de função. Se o aplicativo não for baseado em JavaScript, revise a [código de amostra em outros idiomas](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/JWT/samples/samples.md) para criar um JWT a partir das credenciais de serviço e trocá-lo por um token de acesso com o Adobe IMS.
...
Depois que as Credenciais de serviço são lidas, elas são usadas para gerar um JWT que é trocado com as APIs do Adobe IMS por um token de acesso, que pode ser usado para acessar AEM as a Cloud Service.

Este aplicativo de exemplo é baseado em Node.js, portanto, é melhor usar [@adobe/jwt-auth](https://www.npmjs.com/package/@adobe/jwt-auth) módulo npm para facilitar a (1) geração de JWT e (20 troca com o Adobe IMS. Se o seu aplicativo for desenvolvido usando outra língua, reveja [as amostras de código adequadas](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/JWT/samples/samples.md) sobre como criar a solicitação HTTP para o Adobe IMS usando outras linguagens de programação.
...
1. Atualize o `getAccessToken(..)` para inspecionar o conteúdo do arquivo JSON e determinar se ele representa um token de acesso de desenvolvimento local ou credenciais de serviço. Isso pode ser facilmente feito verificando a existência da variável `.accessToken` , que só existe para o Token de Acesso ao Desenvolvimento Local JSON.
   ...
   Se as Credenciais de Serviço forem fornecidas, o aplicativo gerará um JWT e o intercambiará com o Adobe IMS para um token de acesso. Usaremos o [@adobe/jwt-auth](https://www.npmjs.com/package/@adobe/jwt-auth)&#39;s `auth(...)` que gera um JWT e o troca por um token de acesso em uma única chamada de função.  Os parâmetros para `auth(..)` é um [Objeto JSON composto de informações específicas](https://www.npmjs.com/package/@adobe/jwt-auth#config-object) disponível no JSON de credenciais de serviço, conforme descrito abaixo no código.
   ...
   Agora, dependendo de qual arquivo JSON - o JSON do Token de Acesso ao Desenvolvimento Local ou o JSON de Credenciais do Serviço - é transmitido por meio dessa `file` parâmetro de linha de comando, o aplicativo vai derivar um token de acesso.
...
1. Com essas alterações em vigor, e o JSON de credenciais de serviço baixado do Console do desenvolvedor do AEM (e para simplificar, salvo como `service_token.json` a mesma pasta que esta `index.js`), execute o aplicativo substituindo o parâmetro da linha de comando `file` com `service_token.json`e atualize o `propertyValue` para um novo valor, de modo que os efeitos sejam aparentes em AEM.
...
1. Navegue até a pasta de ativos especificada pela `folder` parâmetro de linha de comando, por exemplo __WKND__ > __Inglês__ > __Aventuras__ > __Brinde de Vinho Napa__
