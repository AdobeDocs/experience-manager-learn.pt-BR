...
+ __Causa:__ O ambiente do público alvo do gasoduto está num estado transitório, no qual não pode aceitar novas construções.
+ __Resolução:__ Aguarde o estado ser resolvido para um estado em execução (ou atualize o estado disponível). Se o ambiente estiver sendo excluído, recrie-o ou escolha um ambiente diferente para o qual criar.
...
+ __Resolução:__ Recrie a ramificação Git ausente usando exatamente o mesmo nome, ou reconfigure o pipeline para ser construído a partir de uma ramificação diferente existente.
...
+ Uma dependência maven não disponível no [Maven Central](https://search.maven.org/) é usada, e o repositório Maven que contém a dependência é:
   ...
   + Não explicitamente registrado no projeto `pom.xml`. Observe que, a inclusão de repositórios Maven é desencorajada, pois aumenta os tempos de construção.
+ Os testes de unidade falharam devido a problemas de tempo. Isso pode ocorrer quando os testes de unidade são sensíveis ao tempo. Um indicador forte está confiando `.sleep(..)` no código de teste.
...
Para resolver problemas de digitalização de código, baixe o relatório formatado em CSV fornecido pelo Cloud Manager por meio do botão Detalhes **de** download e reveja quaisquer entradas.

Para obter mais detalhes, consulte AEM regras específicas, consulte Regras [de verificação de código específicas de AEM](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/custom-code-quality-rules.html)personalizadas de documentações do Cloud Manager.
...
+ __Causa:__ O pacote AEM, todos os pacotes do projeto, contém vários pacotes de código, e a mesma configuração OSGi é fornecida por mais de um dos pacotes de código, resultando em um conflito, resultando na etapa Construir imagem incapaz de decidir qual deve ser usado, falhando a compilação. Observe que isso não se aplica às configurações de fábrica OSGi, desde que tenham nomes exclusivos.
+ __Resolução:__ Analise todos os pacotes de códigos (incluindo quaisquer pacotes de códigos de terceiros incluídos) que estão sendo implantados como parte do aplicativo AEM, procurando configurações OSGi de duplicados que resolvem, por meio do modo de execução, para o ambiente de públicos alvos. A orientação da mensagem de erro de &quot;definir o sinalizador mergeConfigurations como true&quot; não é possível em AEM como um serviço da Cloud e deve ser ignorada.

#### Causa 2

+ __Causa:__ O AEM do projeto inclui incorretamente o mesmo pacote de códigos duas vezes, resultando na duplicação de qualquer configuração OSGi contida nesse pacote.
+ __Resolução:__ Revise todos os pacotes pom.xml incorporados no projeto inteiro e verifique se eles têm a `filevault-package-maven-plugin` configuração [definida como](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#cloud-manager-target) `<cloudManagerTarget>none</cloudManagerTarget>`.
...
+ __Causa:__ Um script repoinit está malformado. Observe que isso pode deixar o repositório em um estado incompleto, já que qualquer script reapontado após a falha será executado no repositório.
+ __Resolução:__ Analise o início rápido local do SDK do AEM quando a configuração OSGi do script de redirecionamento for implantada para determinar se e quais são os erros.
...
Familiarize-se com [AEM como Cloud Service logs](./logs.md) antes de depurar a etapa Implantar. O `aemerror` log contém informações sobre o start e o desligamento de pods que podem ser pertinentes para a implantação de problemas. Observe que o log disponível por meio do botão Download Log na etapa Implantar para do Cloud Manager não é o `aemerror` log e não contém informações detalhadas relacionadas ao start de seus aplicativos.
...
+ __Causa:__ O código personalizado pode executar operações, como query grandes ou conversas de conteúdo, acionadas precocemente em ciclos de vida do pacote OSGi ou do componente, atrasando significativamente o tempo de inicialização do start de AEM.
+ __Resolução:__ Analise a implementação para obter o código que é executado no início do ciclo de vida do pacote OSGi e analise os `aemerror` logs para os serviços de autor e publicação do AEM no momento da falha (tempo de log no GMT), conforme mostrado pelo Gerenciador de nuvem, e procure mensagens de log indicando quaisquer processos de execução de log personalizados.
...
+ __Causa:__ O código personalizado pode chamar operações longas, como query grandes ou traversais de conteúdo, acionados precocemente no pacote OSGi ou ciclos de vida do componente, atrasando significativamente o tempo de inicialização do start de AEM.
+ __Resolução:__ Revise os `aemerror` registros dos serviços de autor e publicação do AEM na hora (tempo de registro em GMT) da falha, conforme mostrado pelo Gerenciador de nuvem.
...
`/var` é silencioso que contém uma variedade de conteúdo temporário em tempo de execução. Incluir `/var` em pacotes de conteúdo (por exemplo, `ui.content`) implantado pelo Cloud Manager pode fazer com que a etapa Implantar falhe.
...
1. Determinar que pelo menos um pacote de conteúdo que faz parte da implantação é gravado em `/var`.
...
+ __Resolução:__ As seguintes maneiras de resolver esses problemas são listadas na ordem de preferência:
   1. Se os `/var` recursos não forem necessários, remova quaisquer recursos `/var` dos pacotes de conteúdo que são implantados como parte do seu aplicativo.
   2. Se os `/var` recursos forem necessários, defina as estruturas do nó usando o [comando repoinit](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit). Os scripts de redirecionamento podem ser direcionados para Autor de AEM, Publicação de AEM ou ambos, por meio de modos de execução OSGi.
   3. Se os `/var` recursos forem necessários apenas para AEM autor e não puderem ser razoavelmente modelados usando [repontar](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit), mova-os para um pacote de conteúdo discreto, que só é instalado no autor de AEM [incorporando](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) -o ao pacote em uma pasta de modo de execução de autor de AEM ( `all``<target>/apps/example-packages/content/install.author</target>`).
