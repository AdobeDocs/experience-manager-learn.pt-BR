---
title: Principais recursos do assistente de IA no AEM
description: Descubra como o Assistente de IA simplifica as tarefas do AEM para autores, desenvolvedores e administradores com orientação instantânea, exemplos de código e solução de problemas.
version: Experience Manager as a Cloud Service
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Feature Video
duration: 285
last-substantial-update: 2026-03-05T00:00:00Z
jira: KT-20476
source-git-commit: bf6a6f467c3ded713552e0077b13c41fb45511cb
workflow-type: tm+mt
source-wordcount: '61'
ht-degree: 0%

---


# Principais recursos do Assistente de IA

Saiba como o Assistente de IA acelera os fluxos de trabalho do AEM fornecendo orientação instantânea, trechos de código e etapas de solução de problemas. Isso reduz o esforço de suporte e ajuda as equipes a fornecer conteúdo mais rapidamente e com mais qualidade.

>[!VIDEO](https://video.tv.adobe.com/v/3480864/?captions=por_br&learn=on&enablevpops)