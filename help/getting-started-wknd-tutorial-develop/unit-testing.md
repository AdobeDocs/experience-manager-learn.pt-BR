...
1. Saiba mais sobre estruturas e ferramentas usadas frequentemente para testar o código do AEM.
1. Entenda as opções de zombar ou simular recursos do AEM ao gravar testes de unidade.

## Segundo plano {#unit-testing-background}

Neste tutorial, exploraremos como escrever [Testes de Unidade](https://en.wikipedia.org/wiki/Unit_testing) para o [Modelo do Sling](https://sling.apache.org/documentation/bundles/models.html) do nosso componente Byline (criado no [Criação de um Componente do AEM personalizado](custom-component.md)). Os testes de unidade são testes de tempo de criação gravados em Java que verificam o comportamento esperado do código Java. Normalmente, cada teste de unidade é pequeno e valida a saída de um método (ou unidades de trabalho) em relação aos resultados esperados.

Usaremos as práticas recomendadas do AEM e:
...
[O Adobe Cloud ](https://docs.adobe.com/content/help/pt-BR/experience-manager-cloud-manager/using/introduction-to-cloud-manager.html) Manager integra execução de teste de unidade e  [relatórios de cobertura de ](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/understand-your-test-results.html#code-quality-testing) código ao pipeline de CI/CD para ajudar a incentivar e promover as práticas recomendadas de teste de unidade do código AEM.
...
## Inspecionar as dependências de Maven de teste {#inspect-the-test-maven-dependencies}
...
1. Estrutura de teste de Mocks do AEM (por io.wcm)

As dependências de teste **JUnit5**, **Mockito** e **Mocks AEM** são automaticamente adicionadas ao projeto durante a configuração usando o arquétipo [AEM Maven](project-setup.md).
...
1. Crie um teste de unidade para o `BylineImpl.java` fazendo uma nova classe Java em `src/test/java` em uma estrutura de pasta de pacote Java que espelhe a localização da classe Java a ser testada.
   ...
   * `src/test/java/com/adobe/aem/guides/wknd/core/models/impl/BylineImplTest.java`
   ...
   Mas também diferencie o arquivo de teste    O sufixo `Test` no arquivo de teste da unidade, `BylineImplTest.java` é uma convenção que nos permite
...
Neste tutorial, a última abordagem é usada (como já criamos um **BylineImpl.java** em um capítulo anterior). Por isso, devemos rever e entender os comportamentos dos seus métodos públicos, mas também alguns dos seus detalhes de implementação. Isso pode parecer contrário, uma vez que um bom teste só deve se preocupar com os fatores de produção e os resultados, contudo, ao trabalhar no AEM, há uma variedade de considerações de implementação que precisam ser compreendidas para construir testes de trabalho.

A TDD no contexto do AEM requer um nível de conhecimento e é melhor adotada pelos desenvolvedores do AEM com proficiência no desenvolvimento do AEM e no teste de unidade do código AEM.

## Configuração do contexto de teste do AEM {#setting-up-aem-test-context}

A maioria dos códigos escritos para o AEM depende das APIs JCR, Sling ou AEM, que, por sua vez, exigem que o contexto de um AEM em execução seja executado corretamente.

Como os testes de unidade são executados na criação, fora do contexto de uma instância do AEM em execução, não há esse contexto. Para facilitar isso, os [mocks do AEM do wcm.io](https://wcm.io/testing/aem-mock/usage.html) criam um contexto de modelo que permite que essas APIs _principalmente_ atuem como se estivessem em execução no AEM.
...
1. Crie um contexto AEM usando **wcm.io&#39;s** `AemContext` em **BylineImplTest.java** adicionando-o como uma extensão JUnit decorada com `@ExtendWith` no arquivo **BylineImplTest.java**. A extensão cuida de todas as tarefas de inicialização e limpeza necessárias. Crie uma variável de classe para `AemContext` que pode ser usada para todos os métodos de teste.
   ...
   Essa variável, `ctx`, expõe um modelo de contexto do AEM que fornece várias abstrações do AEM e Sling:
   ...
   * Fornece uma variedade de modelos de objetos e ajuda comuns necessários, como objetos SlingHttpServletRequest , uma variedade de modelos de serviços Sling e AEM OSGi, como ModelFactory, PageManager, Page, Template, ComponentManager, ComponentManager, Component, TagManager, Tag, etc.
...
1. No método `setUp(..)`, que é executado antes de cada método `@Test`, defina um estado de teste de modelo comum:
   ...
   * **`addModelsForClasses`** registra o Modelo do Sling a ser testado, no modelo do AEM Context, para que ele possa ser instanciado nos  `@Test` métodos .
