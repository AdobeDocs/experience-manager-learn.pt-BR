...
Este artigo mostra como criar vários tipos de PDFs. Ajuda você a entender como e por usar cada um. Todos esses tipos funcionam melhor na principal ferramenta de cliente para visualizar e trabalhar com PDFs — Adobe Acrobat DC.
...
>[!NOTE]
>
> É possível criar PDFs estáticos usando o AEM Forms Designer, salvando o XDP como Formulário PDF estático Adobe
...
O Adobe usa o termo formulário PDF para fazer referência aos formulários interativos e dinâmicos criados com o AEM Forms Designer. É importante observar que há outro tipo de formulário PDF, chamado Acroform, que é diferente das PDF forms criadas no AEM Forms Designer. Os formulários e arquivos criados com o Designer são baseados na XML Forms Architecture (XFA) do Adobe. De muitas formas, o formato de arquivo PDF XFA está mais próximo de um arquivo HTML do que de um arquivo PDF tradicional. Por exemplo, o código a seguir mostra a aparência de um objeto de texto simples em um arquivo PDF XFA.
...
Os PDF forms XFA estáticos não alterarão seu layout no tempo de execução, mas podem ser interativos para o usuário. Estas são algumas vantagens dos PDF forms XFA estáticos:

* Os PDF forms XFA estáticos não alterarão seu layout no tempo de execução, mas podem ser interativos para o usuário.
* Os formulários estáticos são compatíveis com as ferramentas Comentário e Marcação da Acrobat.
* Os formulários estáticos permitem importar e exportar comentários do Acrobat.
* Os formulários estáticos suportam a subconfiguração de fonte, que é uma técnica que pode ser feita em um servidor AEM Forms.
...
* Ao usar o Acrobat Distiller para converter arquivos Postscript e EPS (Encapsulated Postscript) em PDFs: O Distiller geralmente é usado em publicações de impressão e outros fluxos de trabalho que exigem uma conversão do formato Postscript para o formato PDF
...
As formas são a tecnologia de forma interativa mais antiga da Adobe; elas datam de Acrobat versão 3. O Adobe fornece o [Acrobat Forms API Reference](assets/FormsAPIReference.pdf), de maio de 2003, para fornecer os detalhes técnicos desta tecnologia. Acroformes são uma combinação do
seguintes itens:
...
Embora os documentos tenham a mesma aparência, o arquivo PDF/A é aberto com uma barra azul na parte superior, indicando que você está visualizando esse documento no modo PDF/A. Essa barra azul é a barra de mensagens do documento do Acrobat, que você vê ao abrir determinados tipos de arquivos PDF.
...
>[!NOTE]
>
> Se clicar em Ativar edição, retire este documento da conformidade com PDF/A.
